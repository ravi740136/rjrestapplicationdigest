...
public class MyRestAppController {
	...
	public ResponseEntity<Movie> updateMovieByPatch(@PathVariable int id, @RequestBody Movie data) throws InvalidMovieException {
		Optional<Movie> m = repository.findById(id);
		if (!m.isPresent()) {
			throw new InvalidMovieException("movie with specified id " + id + " does not exist ");
		}
	...
	public ResponseEntity<Movie> updateMovie(@PathVariable int id, @RequestBody Movie data) throws InvalidMovieException {
		Optional<Movie> m = repository.findById(id);
		if (!m.isPresent()) {
			throw new InvalidMovieException("movie with specified id " + id + " does not exist ");
		}
